...
namespace stinky {
    ...
    void cube_texture::LoadFromSingleFile() {
        ...
        byte *data = stbi_load(_M_path.c_str(), &srcWidth,
            ...
            for (uint32_t cx = 0; cx < 4; cx++) {
                ...
                for (uint32_t y = 0; y < _M_face_height; y++) {
                    ...
                    for (unsigned x = 0; x < _M_face_width; x++) {
                        ...
                        cubeTextureData[face][(x + y * _M_face_width) * stride + 0] = data[(xp + yp * srcWidth) * stride +
                                                                                           0];
                        cubeTextureData[face][(x + y * _M_face_width) * stride + 1] = data[(xp + yp * srcWidth) * stride +
                                                                                           1];
                        cubeTextureData[face][(x + y * _M_face_width) * stride + 2] = data[(xp + yp * srcWidth) * stride +
                                                                                           2];
                        if (stride >= 4)
                            cubeTextureData[face][(x + y * _M_face_width) * stride + 3] = data[
