...
namespace stinky {
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
    platform_independent_window::platform_independent_window(const WindowProperties &properties,
    ...
    void platform_independent_window::Init() {
        ...
        glfwSetWindowCloseCallback(m_Window, [](GLFWwindow *window) {
                                       ...
                                       data.eventController.OnEvent(window_close_event());
                                   }
        );
        ...
        glfwSetScrollCallback(m_Window, [](GLFWwindow *window, double xOffset, double yOffset) {
            ...
            data.eventController.OnEvent(mouse_scrolled_event((float) xOffset, (float) yOffset));
        });

        glfwSetCursorPosCallback(m_Window, [](GLFWwindow *window, double xPos, double yPos) {
            WindowData &data = *(WindowData *) glfwGetWindowUserPointer(window);

            data.eventController.OnEvent(mouse_moved_event((float) xPos, (float) yPos));
        });
        ...
        glfwSetMouseButtonCallback(m_Window, [](GLFWwindow *window, int button, int action, int mods) {
            ...
            switch (action) {
                case GLFW_PRESS: {
                    data.eventController.OnEvent(mouse_button_pressed_event(static_cast<mouse_code>(button)));
                    break;
                }
                case GLFW_RELEASE: {
                    data.eventController.OnEvent(mouse_button_released_event(static_cast<mouse_code>(button)));
                    break;
                }
            }
        });

        _M_event_controller.OnEvent(glfw_window_post_init_event(m_Window));
    }
